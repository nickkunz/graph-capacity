...
URL_AMAZON = "https://snap.stanford.edu/data/finefoods.txt.gz"

## load amazon network
def load_network_amazon(url: str, root_path: str, timeout: int = 30) -> pd.DataFrame:
    
    ## configure path
    file_name = os.path.basename(url)
    path_dir = os.path.join(root_path, 'amazon')
...
class AmazonProcessor:
    def __init__(self, root_path: str, url: str = URL_AMAZON):
    ...
    def load_data(self):
        ...
        self.data_raw = load_network_amazon(url = self.url, root_path = self.root_path)
        return self
    ...
    def process_network(self):
        ...
        # Selectively compute invariants to avoid memory-intensive spectral calculations
        calculator = GraphInvariants(graph = self.graph)
        invariants = {}
        invariants.update(calculator.simple())
        invariants.update(calculator.cohesion())
        invariants.update(calculator.extremal())
        invariants.update(calculator.statistical())
