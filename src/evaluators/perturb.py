...
def network_perturb(
    ...
    """
    Modifies graph topology G and recomputes invariants x(G').
    ...
    if method == "rewire":
        ...
        G.rewire(n = n_swaps, mode = "simple", multiple = False, loops = False)
    ...
    inv = GraphInvariants(G)
    return {
        "invariants": inv.all(),
        "delta_edges": G.ecount() - graph.ecount()
    }
